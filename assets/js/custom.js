...
function submitMdpOublie()
{

var form=$("#form-mdpOublie");

$("#successEmailOublie").hide();
$("#errorEmailOublie").hide();
...
$("#loaderEmailOublie").show();
           
     },  
 success: function(json) {

 $("#loaderEmailOublie").hide();
if(json.errorEmailOublie === 'yes') {
$("#successEmailOublie").show();


} 
else
{
$("#errorEmailOublie").show();
$("#errorEmailOublie").html(json.errorEmailOublie);
...
function setLanguage(lang)
{
 ...
 $.ajax({
  type: 'POST',
  url: 'https://leskits.com/cookieLanguage.php',
  data: {lang: lang},
  dataType: 'json', // JSON
...
function afficherShop(id)
{

 $.ajax({
  type: 'POST',
  url: 'https://leskits.com/ajaxRequest/afficherShop.php',
  data: {idObjet: id},
  dataType: 'json', // JSON
...
function creerUnKitIndexDeco()
{
$("#connexionLi").trigger('click');
}

// Formulaire création de Kit

function submitFormCreationKit()
{

var form=$("#form-creerKit");

$("#successCreerKit").hide();
$("#errorCreerKit").hide();
...
if(json.erreurCreerKit === 'yes') {
// Redirection vers le kit
$("#successCreerKit").show();
$(form).closest('form').find("input[type=text], textarea").val("");

  window.setTimeout(function(){

        window.location.href = "index.php?page=kitIndividuel&idKit=" + json.id;
...
{
$("#errorCreerKit").show();
$("#errorCreerKit").html(json.erreurCreerKit);
...
$('#affichageObjet').on('hidden.bs.modal', function () {
  changeUrlObjet();
})


function changeUrlObjet()
{
var currentUrl = window.location.href;
var url = currentUrl.split('/').slice(-2)[0];
url = "https://leskits.com/kit/" + url + "/"; 

window.history.pushState({page: "Setup"}, "setup", url);

}

function afficherObjetIndividuel(idKit, idObjet, url)
{
var n = url.length;
if(n > 3)
{
var currentUrl = window.location.href;
var url2 = currentUrl.split('/').slice(-2)[0]; 
url2 = "https://leskits.com/kit-objet/" + url2 + "/" +  url + "-" + idObjet; 
...
{
var currentUrl = window.location.href;  
url2 = currentUrl.substring(0, currentUrl.length - 1);   
}

window.history.pushState({page: "Setup"}, "setup", url2);




$("#idObjetComment").val(idObjet);

 $.ajax({
  type: 'POST',
  url: 'https://leskits.com/ajaxRequest/afficherObjetIndividuel.php',
  data: {idKit: idKit, idObjet: idObjet},
  dataType: 'html', // JSON
     
beforeSend: function() {

$("#loadObjetIndividuel").show();
           
     },  
 success: function(html) {

$("#loadObjetIndividuel").hide();
$("#affichageObjet").modal('show');
$("#contentObjetIndividuel").html(html);
...
var idKit = $("#idKit").val();
 ...
 $.ajax({
  type: 'POST',
  url: 'https://leskits.com/ajaxRequest/afficherObjetsKits.php',
  data: {idKit: idKit, adblock: adBlockEnabled},
  dataType: 'html', // JSON
     
beforeSend: function() {

$("#listeObjets").html('<center><img src="https://leskits.com/assets/img/loader.svg" id="loaderAjoutCommentaire"></center>');
           
     },  
 success: function(html) {


 $("#listeObjets").html(html);
